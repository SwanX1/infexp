...
public class GlowstoneCanyonRuinStructure extends IEStructure<NoneFeatureConfiguration> {
	public GlowstoneCanyonRuinStructure(Codec<NoneFeatureConfiguration> codec) {
		super(codec);
	}
	...
	public StructureFeatureConfiguration getSeparationSettings() {
		return new StructureFeatureConfiguration(2, 1, 20394857);
	}
		...
		public void generatePieces(RegistryAccess dynamicRegistryManager, ChunkGenerator chunkGenerator, StructureManager templateManager, int chunkX, int chunkZ, Biome biome, NoneFeatureConfiguration config) {
			...
			if (pos.getY() != 0) {
				JigsawPlacement.addPieces(
						...
						new JigsawConfiguration(() -> dynamicRegistryManager.registryOrThrow(Registry.TEMPLATE_POOL_REGISTRY).get(new ResourceLocation(InfernalExpansion.MOD_ID, "glowstone_canyon_ruin/start_pool")), 1),
